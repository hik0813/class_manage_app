...
> **주의: `index.html`을 더블클릭해서 열면 안 됩니다!**
...
5. GitHub Pages에는 서버가 없으므로 AI 설명은 **규칙 기반(기본)** 으로 동작합니다. 키가 없어도 모든 기능이 정상 동작합니다.
...
이 앱의 "왜 추천하나요?" 설명은 **2단계 폴백** 구조라 어떤 환경에서도 항상 채워집니다.

| 우선순위 | 방식 | 조건 |
|---|---|---|
| 1 | **Netlify 서버리스 (방식 A)** | Netlify 배포 + 환경변수 `ANTHROPIC_API_KEY` 설정 시 자동 동작 |
| 2 | **규칙 기반 설명 (방식 C, 기본)** | 아무 설정 없이 항상 동작. 사용자 조건 + 서비스 정보를 조합한 문장 생성 |
